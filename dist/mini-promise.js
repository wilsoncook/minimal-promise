...
(function (factory) {
    ...
    var MiniPromise = (function () {
        ...
        //立即执行任务函数
        MiniPromise.prototype.resolveTask = function (fn) {
            var _this = this;
            ...
            try {
                fn(function (result) {
                    ...
                    // this.resolve(result);
                    MiniPromise.nextTick(function () { return _this.resolve(result); }); //放到下一个事件循环中执行（此举在于支持promise被立即解决后，在当前代码段(当前事件循环)中，还可在后面继续添加then操作）
                }, function (error) {
                    if (done) {
                        return;
                    }
        ...
        MiniPromise.prototype.resolve = function (result) {
            if (MiniPromise.isThenable(result)) {
         ...
         */
        MiniPromise.prototype.complete = function (fulfilled, value) {
        ...
        MiniPromise.prototype.fulfill = function (result) { this.complete(true, result); };
        MiniPromise.prototype.reject = function (error) { this.complete(false, error); };
        //添加一个结果回调（同其他库的done）
        MiniPromise.prototype.addHandler = function (onFulfilled, onRejected) {
            var _this = this;
            MiniPromise.nextTick(function () {
        ...
        //某对象是否是promise
        MiniPromise.isThenable = function (thenable) {
            return 'object' === typeof thenable && 'function' === typeof thenable.then;
        };
        MiniPromise.resolve = function (result) {
        ...
        MiniPromise.reject = function (error) {
            this.checkBadClass(this);
            return new this(function (resolve, reject) { return reject(error); });
        };
        MiniPromise.defer = function () {
        ...
        MiniPromise.all = function (iterable) {
            this.checkBadClass(this);
        ...
        MiniPromise.race = function (iterable) {
            var _this = this;
            this.checkBadClass(this);
            if (!this.isIterable(iterable)) {
                return this.reject(new TypeError('Supplied parameter should be iterable.'));
        ...
        MiniPromise.checkBadClass = function (cls) {
            if (cls !== MiniPromise) {
                throw new TypeError('Bad promise constructor.');
            }
        };
        MiniPromise.checkBadInstance = function (inst) {
            if (inst.constructor !== MiniPromise) {
                throw new TypeError('Bad promise instance.');
            }
        };
        //是否是可遍历的对象（用于all()）
        MiniPromise.isIterable = function (iterable) {
        ...
        MiniPromise.nextTick = function (fn) {
            //node环境
            process.nextTick(fn);
            //other TODO
        };
